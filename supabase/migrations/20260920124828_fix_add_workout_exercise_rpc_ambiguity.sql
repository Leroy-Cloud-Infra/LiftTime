...
begin
  ...
  select existing_we.id
  into v_existing_workout_exercise_id
  from public.workout_exercises as existing_we
  where existing_we.session_id = v_session.id
    and existing_we.exercise_id = v_exercise.id
  limit 1;
  ...
  select coalesce(max(ordered_we.order_index), 0) + 1
  into v_next_order_index
  from public.workout_exercises as ordered_we
  where ordered_we.session_id = v_session.id;
